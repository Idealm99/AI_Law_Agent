...
from schemas.schema import PersonalRagState, ExtractedInformation, RefinedQuestion
from model.tools.langchain_tools import tool_class
from model.prompts.prompt import extract, rewrite, anwer
from model.llm import openai
llm = openai().llm

def retrieve_documents(state: PersonalRagState) -> PersonalRagState:
    print("---문서 검색---")
    query = state.get("rewritten_query", state["question"])
    docs = tool_class.personal_law_search.invoke(query)
